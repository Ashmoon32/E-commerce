...
@section('content')
    ...
    <section class="py-16 md:py-24 text-center">
        ...
        <p class="text-lg text-gray-600 max-w-xl mx-auto mb-8">Curated products, minimal design, fair prices – delivered to
            your door.</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            @foreach($categories as $category)
                <a href="{{ route('shop.index', ['search' => $category->name]) }}"
                    class="block border border-gray-200 p-6 text-center hover:border-black hover:shadow-lg transition duration-300 transform hover:-translate-y-1">
                    <span class="text-4xl mb-2 inline-block">
                        ...
                        @foreach($popularProducts as $product)
                            <div class="w-full flex-shrink-0 md:w-1/3 lg:w-1/4 px-2">
                                <a href="{{ route('shop.show', $product) }}"
                                    ...
                                    <div class="w-full h-40 bg-gray-100 flex items-center justify-center text-gray-400 mb-2">No
                                        Image</div>
        ...
        <section class="mb-16">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                @foreach($latestProducts as $product)
                    <a href="{{ route('shop.show', $product) }}"
                        class="block border border-gray-200 p-3 hover:shadow-lg transition duration-300 transform hover:-translate-y-1">
                        <div class="w-full h-32 bg-gray-100 flex items-center justify-center text-gray-400 mb-2">No Image</div>
